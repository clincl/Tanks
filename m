...
class Controller():
	...
	def game_intro(self):
		...
		self.message_to_screen("Hit the Tanks Before it Hits You",u.BLACK,-30,size="small")
		self.message_to_screen('',u.BLACK,-130,size="small")
	...
	def gameLoop(self):
		...
		while True:
			if self.gameState == "Start":
				...
				self.message_to_screen("Game Over press C to play again or Q to Quit", u.YELLOW,y_displace=-50,size = "large")
				pg.display.update()
			...
			if self.gameState == "Instructions":
				self.message_to_screen("For Player One: W:UP S:DOWN A:LEFT D:RIGHT", u.GREEN,y_displace=-50,size = "medium")
				self.message_to_screen("For Player Two: UP ARROW:UP DOWN ARROW:DOWN LEFT ARROW:LEFT RIGHT ARROW:RIGHT", u.GREEN,y_displace=-50,size = "medium")
				self.message_to_screen("GOOD LUCK!", u.GREEN,y_displace=-50,size = "medium")
				for event in pg.event.get():
				    if event.type == pg.KEYDOWN:
				        if event.key == pg.K_q:
				            sys.exit()
